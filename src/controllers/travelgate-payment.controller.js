...
export const confirmPaymentAndBook = async (req, res) => {
  try {
    ...
    // Propagar influencer si el usuario lo tenía y la booking no
    try {
      ...
      console.warn("(INF) No se pudo propagar influencer a booking TGX:", e?.message || e);
    }
      ...
      try {
        ...
        if (dc.user_id) {
          try {
            ...
            console.warn("(INF) No se pudo registrar evento de influencer:", e?.message || e)
          }
        }
      } catch (e) {
    ...
    if (booking.influencer_user_id) {
      try {
        ...
        console.warn("(INF) No se pudo registrar evento de influencer (referral):", e?.message || e)
      }
    }
