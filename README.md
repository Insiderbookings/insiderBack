...
- Cualquier otra variable requerida por las integraciones utilizadas (ver código fuente para más detalles). Para WebBeds defina `WEBBEDS_USERNAME`, `WEBBEDS_PASSWORD_MD5` (o `WEBBEDS_PASSWORD` para generar el hash), `WEBBEDS_COMPANY_ID`, `WEBBEDS_HOST`, `WEBBEDS_TIMEOUT_MS` y `WEBBEDS_RETRIES` como referencia en `src/providers/webbeds/config.js`.
